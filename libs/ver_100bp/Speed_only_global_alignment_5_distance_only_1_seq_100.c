...
//		printf("%c %c %f\n",A,B,imut[k][t]);
    ...
    int CC[110] = {0,};
    int DD[110] = {0,};
    unsigned long (*TT)[110] = (unsigned long(*)[110])malloc(sizeof(unsigned long)*110*110);
//    int TT[1500][2000] = {{0,},};
    int lenx,leny,i,j,x,y,flag;
    char temp;
    char aligned_seq1[220] = "";
    char aligned_seq2[220] = "";
//    char aligned_sum[8000] = "";
    char* aligned_sum = malloc(330);
...
//    }
	...
	for(i = 1; i<leny-1;i++) {
      ...
      //printf(" %3d\n",CC[j]);
      // vertical right end region // j = lenx-1
      diagonalscore = temp_double + match(aseq[j-1],bseq[i-1]);
      temp_double = CC[j];
      upscore = DD[j];
      leftscore = bleftscore;
